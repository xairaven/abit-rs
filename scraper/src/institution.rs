...
impl<'a> InstitutionContext<'a> {
    ...
    async fn find_all(&self) -> Result<Vec<Institution>, InstitutionError> {
        ...
        for row in rows {
            let category =
                InstitutionCategory::try_from(row.category_id).map_err(|_| {
                    let error = format!("Category. Given number: {}", row.category_id);
                    InstitutionError::InconsistentDictionaryData(error)
                })?;
            let ownership_form =
                OwnershipForm::try_from(row.ownership_form_id).map_err(|_| {
                    let error = format!(
                        "Ownership Form. Given number: {}",
                        row.ownership_form_id
                    );
                    InstitutionError::InconsistentDictionaryData(error)
                })?;
            let region =
                row.region_id
                    ...
                    .transpose()
                    .map_err(|_| {
                        let error = format!("Region. Given number: {:#?}", row.region_id);
                        InstitutionError::InconsistentDictionaryData(error)
                    })?;
